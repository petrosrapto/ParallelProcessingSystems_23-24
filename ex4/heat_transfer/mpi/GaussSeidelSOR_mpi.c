...
/* we make some requests before the calling of the GaussSeidel() and some after. these requests
   are stored at the correspondent arrays. before_status and after_status are the arrays that
   hold the result of the request. we keep count of how many requests we sent using the 
   before_req_len and after_req_len */	
    MPI_Status status;
...
/* below the usage of immediates (send/receive) is necessary in order to avoid deadlocks,
   cause there are circular dependencies (regarding the data that must be sent/received)
   between the processes. in addition to that, non-blocking is faster cause we dont wait
   for a particular transaction to be completed and we proceed to the rest of the workload */

/* we remind that the gaussseidelsor algorithm requires the below transactions between the
   processes of the distributed system. Each process should:
   -Before the calculation of the temperature of the new time frame, send the data of the previous 
   time frame to the upper and left neighbor
   -Before the calculation of the temperature of the new time frame, receive the data of the new (current) time frame from the upper and left neighbor
   -Before the calculation of the temperature of the new time frame, receive the data of the previous time frame from the lower and right neighbor
   -After the calculation of the temperature of the new time frame, send the data of the new time frame to the lower and right neighbor
*/
        /*Fill your code here*/
...
/* 	could we use separate arrays for the requests of the Isend and Irecv in order to wait
	only for the recvs to be successful?
	the typical case is that we wait for both operations to complete.
	keep in mind that Isend doesnt copy the data that it needs to send.
        So we must be sure that the correct data has been sent, in other words that the data array
        hadnt been altered before the Isend was successful. Thats why we use wait and we dont 
	continue */
	...
	/* the usage of wait here is NOT optional. Isend doesnt copy the data that it needs to send.
	   So we must be sure that the correct data has been sent, in other words that the data array
	   hadnt been altered before the Isend was successful. Thats why we use wait and we dont    
           continue */  
