            ...
            double * clusters)        /* out: [numClusters][numCoords] */
{
    ...
    do {
         ...
         */
	err = MPI_Allreduce(rank_newClusterSize, newClusterSize, numClusters, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
	if(err != MPI_SUCCESS) {
		fprintf(stderr, "MPI_Allreduce newClusterSize failed with error code %d\n", err);
        	MPI_Abort(MPI_COMM_WORLD, err);
	}
	err = MPI_Allreduce(rank_newClusters, newClusters, numClusters * numCoords, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
	if(err != MPI_SUCCESS) {
		fprintf(stderr, "MPI_Allreduce newClusters failed with error code %d\n", err);
        	MPI_Abort(MPI_COMM_WORLD, err);
	}
         ...
         */
	err = MPI_Allreduce(&rank_delta, &delta, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
	if(err != MPI_SUCCESS) {
		fprintf(stderr, "MPI_Allreduce delta failed with error code %d\n", err);
        	MPI_Abort(MPI_COMM_WORLD, err);
	}
