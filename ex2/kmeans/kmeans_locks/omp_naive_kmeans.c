            ...
            double * clusters)         /* out: [numClusters][numCoords] */
{
    ...
    do {
         ...
         */
	#pragma omp parallel for private(index,j,i) shared(numObjs,numClusters,numCoords,objects,clusters,membership,newClusterSize,newClusters) \
	 reduction(+:delta) default(none)
        for (i=0; i<numObjs; i++) {
